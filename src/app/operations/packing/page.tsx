...
import { packingJobs } from "@/lib/prototype-data";
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_360px]">
        <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid gap-3">
            {packingJobs.map((job) => (
              <div key={job.order} className="grid grid-cols-[1fr_0.7fr_0.7fr_0.7fr] items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm">
                <p className="font-semibold text-slate-950">{job.order}</p>
                <p>{job.box}</p>
                <p>{job.weight}</p>
                <span className="w-fit rounded-full bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">{job.awb}</span>
              </div>
            ))}
          </div>
        </div>
...
function PackAction({ icon: Icon, title }: { icon: typeof PackageCheck; title: string }) {
  return (
    <button type="button" className="flex w-full items-center gap-4 rounded-[28px] border border-slate-200 bg-white p-5 text-left shadow-sm">
